...
public class IndexList 
{
	...
	public IndexList(ArrayList<Index> list) 
	{
		this.list = list;
	}
	...
	public void printTable(DbmsPrinter out) {
		...
		for (Index index : list) 
		{
			...
			if (index.getKeys().size() > 0) 
			{
				 ...
				 * 		prints high key
				 */
				listOut.append(String.format("%-" + keySize + "s", index.getKeys().get(index.getKeys().size() - 1).value));
			} 
			else 
			{
			...
			if (index.getKeys().size() > 0) 
			{
				 ...
				 * 		prints low key
				 */
				listOut.append(String.format("%-" + keySize + "s", index.getKeys().get(0).value));
			} 
			else 
			{
				listOut.append(String.format("%-5s", "-"));
